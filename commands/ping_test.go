...
func TestPingCommandOK(t *testing.T) {
	result, _ := pingCommand.ProcessText("!ping", userTest, "testchat")
	assert.Equal(t, "pong!!", result, "Ping Command")
}

func TestPingCommandNotMatch(t *testing.T) {

	result, _ := statsCommand.ProcessText("!ping6", userTest, "testchat")
	assert.Equal(t, "", result, "Empty output doesn't match")

	_, err := statsCommand.ProcessText("!ping6", userTest, "testchat")
